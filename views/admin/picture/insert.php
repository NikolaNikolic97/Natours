<div class="col-md-6 col-md-offset-3">
    <form action="PHP/pictureInsert.php" method="post" class="form-horizontal" enctype="multipart/form-data">

        <fieldset>
            <input type="hidden" name="id" value="">
            <div id="legend">
                <legend class="">Insert</legend>
            </div>
            <div class="control-group">
                <!-- Username -->
                <label class="control-label"  for="username">Description</label>
                <div class="form-group">
                    <input type="text" value="" name="desc" id="desc" placeholder="" class="form-control" autofocus>
                </div>
            </div>
            <div class="control-group">
                <div class="form-group">
                    <div class="form-check has-success">
                        <label class="form-check-label">
                            <input type="file" class="form-check-input" id="file" name="file" value="aktivan" />
                            Choose from file
                        </label>
                    </div>
                </div>
            </div>


            <div class="control-group">
                <!-- E-mail -->
                <label class="control-label" for="kategorija">Category</label>
                <div class="form-group">
                    <select name="tour" id="tour" class="form-control">
                        <?php foreach($tura as $p): ?>
                            <option  value="<?= $p->id ?>"> <?= $p->naziv ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>

            <div class="control-group">
                <!-- Button -->
                <div class="controls">
                    <input class="btn btn-success" type="submit" name="insert" id="insert"/>
                </div>
                <div id="feedback" class="text-danger">

                </div>
            </div>
        </fieldset>
    </form>
</div>